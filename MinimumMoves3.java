public class MinimumMoves {
    public static int minMoves(String word) {
        ...
        firstIndex = new int[26]; // reset the array
        for (int i = n - 1; i >= 0; i--) {
            int index = word.charAt(i) - 'a';
        ...
        int minMoves = n; // Initialize minMoves to the length of the word
        for (int i = 0; i < n; i++) {
            int moves = 1; // initial move to delete the character itself
            if (leftOccurrences[i] != 0 || rightOccurrences[i] != 0) moves++; // if there's any occurrence on either side
